...
- **Analysis:** A fix was attempted and applied (adding `RedrawWindow`
  ...
  completed), and confirmed to build cleanly — but the user re-tested twice (including after a
...
- **Suggested handling (not yet implemented):** try forcing an actual geometry change on the
  ...
  invalidation) instead of/in addition to `RedrawWindow` — e.g. toggle the window size by ±1px
  ...
  nudge — this diagnostic question was raised but not yet answered by the user.
